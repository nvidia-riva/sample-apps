...
from config import riva_config, llm_config, LLM_PROMPT_DEFAULT, SAMPLE_QUERIES, SAMPLE_RESPONSES
...
def query_llm(intent, timeinfo, weather_data):
    ...
    # Step 1: Create a connecton object
    # conn = nemollm.Connection(
    #     host=llm_config["API_HOST"],
    #     access_token=llm_config["API_KEY"],
    # )
    conn = nemollm.api.NemoLLM(
    ...
    query ='\nIntent: {intent}\nCondition: {condition}\nPlace: {city}\nTime: {time}\nTemperature: {temperature} C\nHumidity: {humidity} percent\nWind Speed: {wind_speed} mph\n'.format(intent=intent, condition=weather_data["condition"], city=weather_data["city"], time=timeinfo, temperature=weather_data["temperature_c"], humidity=weather_data["humidity"], wind_speed=weather_data["wind_mph"])

    # Add the query to the chat context
    chat_context=[
        {"role": "system", "content": "You are Misty, a creative and funny weather reporter that answers questions about weather."},
        {"role": "user", "content": SAMPLE_QUERIES[0]}, 
        {"role": "assistant", "content": SAMPLE_RESPONSES[0]}, 
        {"role": "user", "content": SAMPLE_QUERIES[1]}, 
        {"role": "assistant", "content": SAMPLE_RESPONSES[1]}, 
        {"role": "user", "content": SAMPLE_QUERIES[2]}, 
        {"role": "assistant", "content": SAMPLE_RESPONSES[2]}, 
        {"role": "user", "content": query}
    ]

    # If the user has asked for verbose output, print the full chat context and current query
    if llm_config["VERBOSE"]:
        for role_content_dictionary in chat_context: 
            line = role_content_dictionary['content']
            if role_content_dictionary['role'] == 'assistant':
                line = 'Misty: ' + line
            print(line)
    ...
    try:
        ...
        #     model_id=llm_config["API_MODEL_NAME"],
        response = conn.generate_chat(
            model=llm_config["API_MODEL_NAME"],
            chat_context=chat_context,
            tokens_to_generate=llm_config["TOKENS_TO_GENERATE"],
            logprobs=False,
            temperature=llm_config["TEMPERATURE"],
            top_p=llm_config["TOP_P"],
            top_k=llm_config["TOP_K"],
            stop=llm_config["STOP_WORDS"],
            random_seed=random.randint(a=0, b=2147483647),
            repetition_penalty=llm_config["REPETITION_PENALTY"],
            beam_search_diversity_rate=llm_config["BEAM_SEARCH_DIVERSITY_RATE"],
            beam_width=llm_config["BEAM_WIDTH"],
            length_penalty=llm_config["LENGTH_PENALTY"]
        )
        llm_response=response["text"]
        # response = openai.ChatCompletion.create(
        #     model=llm_config["API_MODEL_NAME"],
        #     messages=LLM_PROMPT_DEFAULT+query,
        #     tokens_to_generate=llm_config["TOKENS_TO_GENERATE"],
        #     logprobs=False,
        #     temperature=llm_config["TEMPERATURE"],
        #     top_p=llm_config["TOP_P"],
