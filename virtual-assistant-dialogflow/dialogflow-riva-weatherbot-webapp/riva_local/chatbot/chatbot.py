...
from riva_local.dialogflow.dialogflow import DialogflowPipe
...
class ChatBot(object):
    ...
    def __init__(self, user_conversation_index, verbose=False):
        ...
        self.asr = ASRPipe()
        self.dialogflow = DialogflowPipe(user_conversation_index)
        self.tts = TTSPipe()
    ...
    # Function to pipeline dialogflow and tts functionality
    def dialogflow_tts_pipeline(self, text):
        # Request dialogflow for response with user input and sender id
        response_text = self.dialogflow.request_dialogflow_for_question(text)
        if len(response_text) and self.enableTTS == True:
            # Fill tts buffer with dialogflow response text, if enableTTS is set to True
